...
print("INFO: Running process_fqs.sh on SRR081238_1.filt.fastq.gz and SRR081238_2.filt.fastq.gz with read group id NA127501 and sample name NA12750", file=sys.stderr)
command_line = 'srun -c 20 /mnt/data/don/Scripts/process_fqs.sh /mnt/data/1KGenomes_Exomes/SRR081238_1.filt.fastq.gz /mnt/data/1KGenomes_Exomes/SRR081238_2.filt.fastq.gz NA127501 NA12750'
...
infiles = '-I ' + ' -I '.join(analysis_groups[-1])
print("INFO: Running TargetCreator on: " + infiles, file = sys.stderr)
command_line = 'srun -c 15 /cm/shared/plab/apps/JDK/jdk1.8.0_11/bin/java -Xmx70g -jar /cm/shared/plab/apps/GATK/3.2-2/GenomeAnalysisTK.jar -nt 15 -T RealignerTargetCreator -R /mnt/data/reference/hs37d5.fa -o Indelrealigner9.intervals --known /mnt/data/reference/GATK_b37/Mills_and_1000G_gold_standard.indels.b37.vcf --known /mnt/data/reference/GATK_b37/1000G_phase1.indels.b37.vcf ' + infiles
shell_args = shlex.split(command_line)
p = subprocess.Popen(shell_args)
time.sleep(10)
p.wait()
...
process_bin = []
for bam_file in analysis_groups[-1]:
    out_file = re.sub('_sorted_mem.bam', '_realigned.bam', bam_file)
    print("INFO: Running IndelRealigner on: " + bam_file, file=sys.stderr)
    command_line = 'srun -c 5 /cm/shared/plab/apps/JDK/jdk1.8.0_11/bin/java -Xmx5g -jar /cm/shared/plab/apps/GATK/3.2-2/GenomeAnalysisTK.jar -T IndelRealigner -R /mnt/data/reference/hs37d5.fa -targetIntervals Indelrealigner' + '9' + '.intervals --knownAlleles /mnt/data/reference/GATK_b37/Mills_and_1000G_gold_standard.indels.b37.vcf --knownAlleles /mnt/data/reference/GATK_b37/1000G_phase1.indels.b37.vcf -I ' + bam_file + ' -o ' + out_file
    shell_args = shlex.split(command_line)
    p = subprocess.Popen(shell_args)
    process_bin.append(p)
    time.sleep(10) #Job handler#
'''
print("INFO: Running IndelRealigner on: /mnt/data/1KGenomes_Exomes/SRR070516_sorted_mem.bam", file=sys.stderr)
command_line = 'srun -c 5 /cm/shared/plab/apps/JDK/jdk1.8.0_11/bin/java -Xmx5g -jar /cm/shared/plab/apps/GATK/3.2-2/GenomeAnalysisTK.jar -T IndelRealigner -R /mnt/data/reference/hs37d5.fa -targetIntervals Indelrealigner2.intervals --knownAlleles /mnt/data/reference/GATK_b37/Mills_and_1000G_gold_standard.indels.b37.vcf --knownAlleles /mnt/data/reference/GATK_b37/1000G_phase1.indels.b37.vcf -I /mnt/data/1KGenomes_Exomes/SRR070516_sorted_mem.bam -o /mnt/data/1KGenomes_Exomes/SRR070516_realigned.bam'
shell_args = shlex.split(command_line)
p = subprocess.Popen(shell_args)
process_bin.append(p)
time.sleep(10)
...
for group in analysis_groups:
    ...
    infiles = re.sub('sorted_mem.bam', 'realigned.bam', infiles)
    print("INFO: Running HaplotypeCaller on: " + infiles, file=sys.stderr)
    command_line = 'srun -c 15 /cm/shared/plab/apps/JDK/jdk1.8.0_11/bin/java -Xmx55g -jar /cm/shared/plab/apps/GATK/3.2-2/GenomeAnalysisTK.jar -T HaplotypeCaller -R /mnt/data/reference/hs37d5.fa -nct 15 --genotyping_mode DISCOVERY -stand_emit_conf 10 -stand_call_conf 20 -D /mnt/data/reference/GATK_b37/dbsnp_137.b37.vcf -o variants_out' + str(count) + '.vcf -A BaseQualityRankSumTest -A RMSMappingQuality -A TandemRepeatAnnotator -A QualByDepth -A MappingQualityRankSumTest -A ReadPosRankSumTest -A FisherStrand -A Coverage -A MappingQualityZero -A SpanningDeletions ' + infiles
    shell_args = shlex.split(command_line)
    p = subprocess.Popen(shell_args)
    process_bin.append(p)
    time.sleep(10)
