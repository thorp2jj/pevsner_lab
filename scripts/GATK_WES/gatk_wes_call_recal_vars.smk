...
def get_vcf_subsets(wildcards):
    ...
    for start in geno_start:
        infile = "{out}/{tmp}/calls/{start_pos}_combined_raw_wes.vcf"
...
rule genotype:
    input:
        ...
        dbsnp=config['resource']['dbsnp']
    output:
        "{out}/{tmp}/calls/{start_pos}_combined_raw_wes.vcf".format(out=config["dir"]["out"], tmp=config["dir"]["tmp"], start_pos="{start_pos}")
    threads:
        1
    run:
        ...
        "-L /mnt/data/jeremy/projects/sws/collab_samples/sureselect_exon_v5/converted_to_grch37_format/S04380110_Padded_grch37.bed "
...
def get_gvcf_subsets(wildcards):
    ...
    for start in start_pos:
        infile = "{out}/{tmp}/{sample}/{start_pos}_subset_wes.g.vcf"
        infile = infile.format(out=config["dir"]["out"], 
                               tmp=config["dir"]["tmp"],
...
rule call_vars:
    input:
        ...
        ref=config['Reference'],
    output:
        "{out}/{tmp}/{sample}/{start_pos}_subset_wes.g.vcf".format(out=config["dir"]["out"], tmp=config["dir"]["tmp"], sample="{sample}", start_pos="{start_pos}")
    threads:
        1
    run:
        start = str(wildcards.start_pos)
        interval = intervals[str(start)]
        outfile = "{out}/{tmp}/{sample}/{start}_subset_wes.g.vcf".format(out=config["dir"]["out"], tmp=config["dir"]["tmp"], sample="{wildcards.sample}", start=start)
        shell(
